...
class AdminSmartTrickBulkUploadScreen extends StatefulWidget {
  const AdminSmartTrickBulkUploadScreen({super.key});

  @override
  State<AdminSmartTrickBulkUploadScreen> createState() =>
      _AdminSmartTrickBulkUploadScreenState();
}

class _AdminSmartTrickBulkUploadScreenState
    extends State<AdminSmartTrickBulkUploadScreen> {
  _Stage _stage = _Stage.pick;
  String? _fileName;
  List<BulkSmartTrickRow> _rows = [];
  String? _pickError;
  ...
  Future<void> _pickFile() async {
    ...
    try {
      ...
      final rows = <BulkSmartTrickRow>[];
      for (var i = 0; i < parsed.length; i++) {
        rows.add(BulkSmartTrickRow.parse(i + 2, parsed[i], index: index));
      }
      final existing = await FirebaseFirestore.instance
          .collection(SmartTrickRepository.collection)
          .get();
      final existingTricks = existing.docs
          .map((d) =>
              (d.data()['memoryTrick'] as String? ?? '').trim().toLowerCase())
          .where((q) => q.isNotEmpty)
          .toSet();
      final seen = <String>{};
      for (final row in rows) {
        final key = row.memoryTrick.trim().toLowerCase();
        if (key.isEmpty) continue;
        if (existingTricks.contains(key) || seen.contains(key)) {
  ...
  Future<void> _upload() async {
    ...
    try {
      ...
      for (var i = 0; i < toUpload.length; i++) {
        ...
        final ref = firestore.collection(SmartTrickRepository.collection).doc();
        final item = SmartTrickItem(
          id: ref.id,
          title: row.title,
          concept: row.concept,
      ...
      await auditLogRepository.log(
        ...
        details: '${writtenRefs.length} smart trick(s) imported as DRAFT',
      );
      setState(() {
  ...
  Widget build(BuildContext context) {
    return AdminScaffold(
      title: 'Bulk Upload — Smart Tricks',
      body: switch (_stage) {
        _Stage.pick => _pick(),
    ...
    return Center(
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Icon(Icons.psychology_alt_rounded, size: 56, color: AppColors.navy),
            const SizedBox(height: 16),
            const Text(
              'Import Smart Tricks from CSV or Excel\nImported rows stay DRAFT until you publish.',
              textAlign: TextAlign.center,
              style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16),
            ),
            const SizedBox(height: 8),
            Text(
              'Expected columns: ${bulkSmartTrickExpectedHeaders.join(', ')}',
        ...
        Expanded(
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ListTile(
                dense: true,
                title: Text(row.memoryTrick, maxLines: 1, overflow: TextOverflow.ellipsis),
                subtitle: Text(
                  row.isValid
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              '$_uploadedCount smart trick(s) imported as DRAFT.',
