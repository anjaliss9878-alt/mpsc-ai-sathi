...
/// One parsed smart-trick row from CSV/Excel. Always imported as Draft.
class BulkSmartTrickRow {
  BulkSmartTrickRow({
  ...
  factory BulkSmartTrickRow.parse(
    int rowNumber,
    ...
    final memoryTrick =
        cell('memorytrick').isNotEmpty ? cell('memorytrick') : cell('trick');
    final explanation = cell('explanation');
    final example = cell('example');
    ...
    final errors = <String>[];
    if (concept.isEmpty) errors.add('Concept is empty');
    if (memoryTrick.isEmpty) errors.add('Memory trick is empty');
    ...
    return BulkSmartTrickRow(
      ...
      chapter: chapter,
      topic: topic,
      title: title.isNotEmpty ? title : concept,
      concept: concept,
...
const List<String> bulkSmartTrickExpectedHeaders = [
  'Exam',
  'Target Group',
  'Subject',
  'Chapter',
